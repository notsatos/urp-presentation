\documentclass[8pt,reqno]{amsart}
\usepackage{graphicx}
\usepackage{fullpage}
...
\end{itemize}
\begin{theorem}
Let $X$ be a scheme and let $A$ be a ring. To every morphism $f \colon X \to \spec A$, associate the homomorphism: $$R \simeq \Gamma (\spec A, \mathcal{O}_{\spec A}) \xrightarrow{f^\ast} \Gamma (X, \mathcal{O}_X).$$ Then this induces a bijection between $\Hom (X, \spec A)$ in the category of schemes and $\Hom (R, \Gamma (X, \mathcal{O}_X))$ in the category of rings. 	
\end{theorem}
\begin{coro}
The category of affine schemes is equivalent to the category of commutative rings with, with arrows reversed. 	
\end{coro}
\begin{rem}
Let $X$ be any scheme. By Theorem 1.1., there is a unique map $\pi \colon X \to \spec \zz$, induced by 	the unique homomorphism $\pi^\ast \colon \zz \to \Gamma (X, \mathcal{O}_X)$. Moreover, on the level of set theory, we morphism $\pi \colon X \to \spec \zz$ is given by $\pi \colon x \mapsto [\cha (\kappa (x))]$. If we we have $\pi (x) = y$, then we have an inclusion map $$\kappa (y) \xrightarrow{\pi_x^\ast} \kappa (x) = \begin{cases}
 \zz /p \zz 	\\
 \text{or} \\
 \qq 
 \end{cases}
...
\begin{figure}[H]
\includegraphics[scale=.3]{cline.png} 
\caption{A picture of $\spec \cc[x] = \aff^1_\cc .$ Note that we're including $(0)$ to be a prime ideal, however, it is obviously not maximal.   }
\end{figure}


\begin{rem}
What we've done in the previous example is a common example of a generalization: Let $K$ be an algebraically closed field, then we define $\aff^1_K := \spec K[x]$. 	
\end{rem}
\begin{rem}
$\aff^1_{\mathbf{F}_p} = \spec \mathbf{F}_p [x]$. As $\ff_p[x]$ is a Euclidean domain, so the prime ideals of $\ff_p [x]$ are of the form $(0)$ or $(f(x))$ where $f \in \ff_p [x]$ is an irreducible polynomial, which can be of any degree. 
\end{rem}
